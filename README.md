...
| `scylla.custom.window.start` | Is an optional configuration parameter that allows users to define the number of seconds from which the Scylla CDC connector should start reading records. This value represents the time window for reading recent changes in the table. If not specified, the connector will default to reading from the beginning of the TTL, ensuring compatibility with the standard behavior. This parameter provides flexibility for users who need to read only the most recent updates, such as from the last 60 seconds. |
